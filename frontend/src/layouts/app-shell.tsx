...
export function AppShell({
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background text-foreground selection:bg-primary/20">
      ...
      <aside 
        className={cn(
          "relative z-40 flex h-full shrink-0 flex-col border-r border-outline-variant/10 bg-surface-container-low transition-[width] duration-300 ease-in-out",
      ...
      >
        <div className={cn("mb-4 flex h-14 items-center px-4", isSidebarExpanded ? "justify-between" : "justify-center")}>
          <div className="flex items-center">
            ...
            <div className={cn("ml-3 transition-opacity duration-200", isSidebarExpanded ? "opacity-100" : "pointer-events-none opacity-0 w-0 overflow-hidden")}>
              <h1 className="whitespace-nowrap text-sm font-bold tracking-tight text-on-surface">Redmine Pro</h1>
            </div>
          </div>
          
          <button
            onClick={() => setIsSidebarExpanded(!isSidebarExpanded)}
            className="absolute -right-3 top-16 z-50 rounded-full border border-outline-variant/20 bg-surface-container-high p-0.5 text-on-surface-variant shadow-lg transition-all hover:text-primary"
          >
            <MaterialSymbol 
              name="chevron_right" 
        ...
        <nav className="flex flex-1 flex-col gap-1 px-3">
          {APP_ROUTE_ITEMS.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  "group relative flex h-10 items-center overflow-hidden rounded-md px-2 text-[13px] font-medium transition-all",
                  isActive
        ...
        <div className="mt-auto border-t border-outline-variant/10 p-3">
          <div className="flex items-center overflow-hidden px-2 py-3">
            ...
            <div className={cn("ml-3 transition-opacity duration-200", isSidebarExpanded ? "opacity-100" : "pointer-events-none opacity-0 w-0 overflow-hidden")}>
              <p className="truncate whitespace-nowrap text-xs font-bold text-on-surface">{currentUser.name || "管理员"}</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="relative flex flex-1 flex-col h-full bg-surface">
        {/* TopAppBar */}
        <header className="z-30 flex h-14 shrink-0 items-center justify-between border-b border-outline-variant/5 bg-surface px-6">
          <div className="flex items-center gap-8">
            <span className="text-xl font-black tracking-tight text-on-surface">项目管理中心</span>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <Button size="sm" variant="ghost" className="rounded-md p-2 text-on-surface-variant hover:bg-surface-container-high">
                <MaterialSymbol name="history" className="text-[18px]" opticalSize={20} />
              </Button>
              <Button size="sm" variant="ghost" className="relative rounded-md p-2 text-on-surface-variant hover:bg-surface-container-high">
