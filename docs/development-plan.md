...
| 2026-03-26 | 附件媒体预览定位修复 | 已完成 | 问题详情页图片/视频全屏预览改为真正占满视口的弹层容器，修复点击放大后内容偏到右侧的问题，并关闭默认对话框关闭按钮以避免错位叠加 |
| 2026-03-26 | 附件媒体预览关闭交互补齐 | 已完成 | 问题详情页全屏预览新增右上角关闭按钮，并支持点击图片外的遮罩区域直接关闭，避免仅能依赖 ESC 键退出 |
| 2026-03-26 | 附件媒体预览文案收敛 | 已完成 | 去掉预览层右上角的 ESC 与“点击空白处关闭”提示，仅保留关闭按钮，减少视觉干扰；点击空白处关闭能力仍保留 |
| 2026-03-26 | 视频附件预览降级处理 | 已完成 | 全屏预览新增视频兼容性检测；当 WebView 无法拿到可视视频轨或加载失败时，自动切换为下载面板，避免出现只有声音没有画面的假预览状态 |
